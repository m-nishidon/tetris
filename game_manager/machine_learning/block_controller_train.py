...
#            self.model.cuda()
    ...
    def check_cleared_rows(self,board):
        ...
        lines = 0
        empty_line = np.zeros(self.width)
    ...
    def get_real_depth(self, depth):
        depth_ = copy.copy(depth)
        left = np.where(depth_[:5]<=3)[0]
        right = np.where(depth_[5:]<=3)[0]
        if left.size:
            cols = range(np.max(left)+1)
            for col in cols:
                depth_[col] = 0
        if right.size:
            cols = range(np.min(right)+5,10)
            for col in cols:
                depth_[col] = 0
        return depth_
    ...
    # 深さ4以上の井戸の数を求める
    # 特徴量として取得したい高さの合計、穴の数、たかさのばらつきを求める
    def get_well(self, board):
        depth = np.where(board.any(axis=0), np.argmax(board, axis=0), 22)
        # total_depth = depth.sum()
        std_depth = np.std(depth)
        # Iミノをたてにおけない範囲は使えないマスとみなす
        if (depth<=3).sum()>0:
            real_depth = self.get_real_depth(depth)
            modified_depth = True
            total_heights = 220 - real_depth.sum()
        else:
            modified_depth = False
            total_heights = 220 - depth.sum()
        holes = total_heights - board.sum()

        # 井戸を算出
        # 両端に１をつけて拡大
        exp_depth = np.concatenate([[1], depth, [1]])
        dif_left_and_right = np.array([exp_depth[2:], exp_depth[:-2]])
        dif_min = np.max(dif_left_and_right, axis=0)
        # well = np.where(depth>dif_min, depth-dif_min, 0) #井戸の深さを個別に見たい場合
        well_array = np.where(depth > dif_min, depth - dif_min, 0)
        well_count_4 = (well_array >= 4).sum()
        # 4段消し狙い
        # 4段消しが狙える状況に近い場合、高さのばらつきの値を小さくする
        if well_count_4 <= 1 and not modified_depth: # and (well_array > 8).sum()==0:
            if well_count_4 == 1:
                std_depth = np.std(np.delete(depth, np.where(well_array >= 4)))
            else:
                well_count_3 = (well_array >= 3).sum()
                if well_count_3 <= 1:
                    if well_count_3 == 1:
                        std_depth_ = np.std(np.delete(depth, np.where(well_array>=3)))
                        std_depth = std_depth*0.25 + std_depth_*0.75
                    elif (well_array >= 2).sum() == 1:
                        std_depth_ = np.std(np.delete(depth, np.where(well_array>=2)))
                        std_depth = std_depth*0.6 + std_depth_*0.4

        return total_heights, holes, std_depth
    ...
    def get_state_properties(self, board):
        total_heights_cur = 220 - np.where(board.any(axis=0), np.argmax(board, axis=0), 22).sum()
        lines_cleared, board_new = self.check_cleared_rows(board)
        total_heights, holes, std_depth = self.get_well(board_new)
        return torch.FloatTensor([total_heights - total_heights_cur, total_heights, holes, std_depth])
